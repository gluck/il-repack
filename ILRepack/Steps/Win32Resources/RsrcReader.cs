...
namespace ILRepacking.Steps.Win32Resources
{
    internal class RsrcReader
    {
        ...
        public static ResourceDirectory ReadResourceDirectory(string fileName)
        {
            using (var stream = File.Open(fileName, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                var reader = new ImageReader(stream);
                var data = reader.GetSectionData(".rsrc");
                if (data == null)
                {
                    return new ResourceDirectory();
                }
                return ReadResourceDirectory(data.Data, data.VirtualAddress);
            }
        }
    }
}
